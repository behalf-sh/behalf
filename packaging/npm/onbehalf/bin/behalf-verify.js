...
// The `behalf-verify` shim: exec the platform binary with the arguments as given, the
...
try {
  const res = spawnSync(binary('behalf-verify'), process.argv.slice(2), { stdio: 'inherit' })
  if (res.error) throw res.error
  if (res.signal) process.kill(process.pid, res.signal)
  process.exitCode = res.status === null ? 1 : res.status
} catch (err) {
  process.stderr.write(`behalf-verify: ${err.message}\n`)
  process.exitCode = 1
}
